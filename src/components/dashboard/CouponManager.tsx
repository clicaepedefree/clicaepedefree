  ...
  const getErrorMessage = (error: unknown) => {
    ...
    if (error && typeof error === "object") {
      ...
      if (maybeError.code === "23505") {
        return "Já existe um cupom com esse código para este restaurante.";
      }

      if (maybeError.code === "42P01") {
        return "A tabela de cupons ainda não existe no banco. Rode as migrations do Supabase.";
      }

      if (maybeError.code === "42501") {
        return "Sem permissão para criar cupons neste restaurante. Verifique as políticas de acesso (RLS).";
      }
    ...
    return "Erro inesperado ao processar cupom.";
  };
    ...
    try {
      ...
      if (expiresAt && Number.isNaN(new Date(expiresAt).getTime())) {
        toast({
          variant: "destructive",
          title: "Data inválida",
          description: "A data de expiração informada é inválida.",
        });
        setCreating(false);
        return;
      }
